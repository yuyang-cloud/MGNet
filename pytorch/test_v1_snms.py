...
tf_mask = torch.from_numpy(mask).float().requires_grad_(False).cuda()
...
def result_post_process(res, top_number, extracted_frame_num, frame_num, video_name):
    ...
    for j in range(min(top_number, len(df))):
        f_end = int(df.xmax.values[j] * extracted_frame_num)
        f_init = int(df.xmin.values[j] * extracted_frame_num)
...
if __name__ == '__main__':
    ...
    for video_name in tqdm(video_list[:]):
        ...
        res = []
        for s, e in windows:
            input_feat = video_feature[s:e] # 取当前window内的feature: 128,487
    ...
    #     p.join()
        ...
        final_df.to_csv('result_v1_snms.csv', sep=' ', index=False)
