...
class Attention(nn.Module):
    ...
    def forward(self, x, mask):
        ...
        if mask is not None:
            ...
            # mask_in = mask
            # mask_out = ~mask
            # mask_in = rearrange(mask_in, 'b i -> b () i ()') * rearrange(mask_in, 'b j -> b () () j')
            # mask_out = rearrange(mask_out, 'b i -> b () i ()') * rearrange(mask_out, 'b j -> b () () j')
...
class Mask_VisionTransformer(nn.Module):
    """ Vision Transformer
    ...
    def forward(self, x, mask):
        x = self.forward_features(x, mask)    # bs,T,768
