...
def getFullData(video_dict, dbg_config, last_channel=True, training=True):
    ...
    for i in range(len(video_list)):
        ...
        if gt_len_mode == 0:
            ...
            gt_len_small = tgap * gt_len_ratio  # 0.01*2
        ...
        # mask
        gt_len_small = tgap * 2
        gt_start_mask = np.stack(
            (gt_xmins - gt_len_small / 2, gt_xmins + gt_len_small / 2), axis=1
        )
        gt_end_mask = np.stack(
            (gt_xmaxs - gt_len_small / 2, gt_xmaxs + gt_len_small / 2), axis=1
        )
        ...
        # # GT mask
        # tmp_df = pd.read_csv(os.path.join(GT_dir, video_name + ".csv"))
        # GTaction_mask = tmp_df.values[:, 0]    # T
        ...
        batch_mask_start.append(mask_score_start)
        batch_mask_end.append(mask_score_end)
        # batch_GTaction_mask.append(GTaction_mask)
    ...
    dataDict = {
        "gt_action": batch_label_action,  # 19993, 1, 100
        "gt_start": batch_label_start,    # 19993, 1, 100
        "gt_end": batch_label_end,        # 19993, 1, 100
        "mask_start": batch_mask_start,
        "mask_end": batch_mask_end,
        "feature": batch_anchor_feature,  # 19993, 400, 100
        "iou_label": batch_anchor_iou     # 19993, 1, 100, 100  IoU
    }
    if training:
        return dataDict, train_video_mean_len  # train_video_mean_len=19993  每个video所有gt的时长mean
    else:
        return dataDict
