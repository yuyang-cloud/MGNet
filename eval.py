...
def eval():
    ...
    eval_file = 'results/result_proposals.json'

    json_name = 'data/activity_net_1_3_new.json'    # grouth truth json file
    uniform_average_nr_proposals_valid, uniform_average_recall_valid, uniform_recall_valid, AUC = \
        run_evaluation(
            json_name,
            eval_file,
            max_avg_nr_proposals=100,
    ...
    return np.mean(uniform_recall_valid[:, 0]), np.mean(uniform_recall_valid[:, 4]), np.mean(uniform_recall_valid[:, 9]), np.mean(uniform_recall_valid[:, -1]), AUC
...
# d = load_json('./results/result_proposals.json')

# with open("./results/read_result_proposals.json","w") as dump_f:
#     json.dump(d,dump_f, indent=1)