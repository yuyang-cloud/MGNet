...
class DBGConfig():
    ...
    def __init__(self):
        ...
        self.GT_dir = dataset_info['GT_dir']
        ...
        learning_rate = training_info['learning_rate'] # [1e-3, 1e-4]
        lr_epochs = training_info['lr_epochs']         # [8, 4]
        assert len(learning_rate) == len(lr_epochs)
        self.learning_rate = []
        for lr, n in zip(learning_rate, lr_epochs):
            self.learning_rate.extend([float(lr)] * n) # self.learning_rate = [8个0.001  4个0.0001]
        self.epoch_num = len(self.learning_rate) # 8+4=12
